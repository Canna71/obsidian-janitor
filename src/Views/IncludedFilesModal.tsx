...
export class IncludedFilesModal extends Modal {
	...
	render() {
		...
		this.root.render(
			<React.StrictMode>
				<IncludedFilesView
					filters={this.settings.includedFilesFilters}
					folders={folders}
					onCancel={() => { this.close() }}
...
const IncludedFilesView = ({ filters, folders, onCancel, onFilterChanged }: IncludedFilesViewProps) => {
	...
	return <div ref={ref}>
		<div>Files matching the following regular expressions are always included, even if excluded by other rules:</div>
		{list.map((filter, i) => (
